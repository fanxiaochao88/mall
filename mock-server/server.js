...
// Token验证中间件
const authPaths = ['/api/v1/users/me', '/api/v1/carts', '/api/v1/orders', '/api/v1/addresses']
server.use((req, res, next) => {
...
// 发送验证码
server.post('/api/v1/users/send-code', (req, res) => {
...
// 更新用户信息
server.put('/api/v1/users/me', (req, res) => {
...
// 获取首页轮播图
server.get('/api/v1/banners', (req, res) => {
  const banners = [
...
server.get('/api/v1/products', (req, res) => {
  const { page = 1, page_size = 20, category_id, keyword, sort_by = 'default' } = req.query
...
// 更新购物车
server.put('/api/v1/carts/:id', (req, res) => {
  const { id } = req.params
...
// 删除购物车
server.delete('/api/v1/carts/:id', (req, res) => {
  const { id } = req.params
...
// 批量删除
server.post('/api/v1/carts/batch-delete', (req, res) => {
  const { ids } = req.body
...
// 获取订单详情
server.get('/api/v1/orders/:id', (req, res) => {
  const { id } = req.params
...
// 支付订单
server.post('/api/v1/orders/:id/pay', (req, res) => {
  const { id } = req.params
  const order = orders.find(o => o.id === parseInt(id))
...
// 取消订单
server.post('/api/v1/orders/:id/cancel', (req, res) => {
  const { id } = req.params
  const order = orders.find(o => o.id === parseInt(id))
...
// 确认收货
server.post('/api/v1/orders/:id/confirm', (req, res) => {
  const { id } = req.params
  const order = orders.find(o => o.id === parseInt(id))
...
// 获取地址列表
server.get('/api/v1/addresses', (req, res) => {
  res.json({
    code: 200,
    message: '成功',
    data: addresses
...
// 创建地址
server.post('/api/v1/addresses', (req, res) => {
  const address = {
    id: addressIdCounter++,
    user_id: 1,
    ...req.body,
...
// 更新地址
server.put('/api/v1/addresses/:id', (req, res) => {
  const { id } = req.params
...
// 删除地址
server.delete('/api/v1/addresses/:id', (req, res) => {
  const { id } = req.params
...
// 设置默认地址
server.post('/api/v1/addresses/:id/set-default', (req, res) => {
  const { id } = req.params
...
server.post('/api/v1/upload/image', (req, res) => {
  // 模拟上传成功
  res.json({
    code: 200,
    message: '上传成功',
...
server.listen(PORT, () => {
  ...
  console.log(`📚 接口文档: http://localhost:${PORT}/api/v1`)
  console.log(`\n✅ 可用的接口:`)
  ...
  console.log(`  - GET  /api/v1/carts`)
  console.log(`  - GET  /api/v1/orders`)
  console.log(`  - GET  /api/v1/addresses`)
})
